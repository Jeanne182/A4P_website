...
function description_outil_une_variable($index, &$outil, &$variable, &$label, &$modif) {
	...
	$cs_variable['disabled'] = $disab = autoriser('configurer', 'variable', 0, NULL, array(
		'nom' => $cs_variable['nom'],
		'outil' => $outil
	)) ? '' : ' disabled="disabled"';
	...
	if (is_array($radios = &$cs_variable['radio'])) {
		...
		$res = "$label <ul>";
		$i   = 0;
		$nb  = isset($cs_variable['radio/ligne']) ? intval($cs_variable['radio/ligne']) : 0;
		foreach ($radios as $code => $traduc) {
			$br     = (($nb > 0) && (++$i % $nb == 0)) ? '</ul><ul>' : '';
			$traduc = strncmp($traduc, '_', 1) === 0 ? substr($traduc, 1) : _T($traduc);
			$res .= "<li><label><input id=\"label_{$variable}_$code\" class=\"cs_input_checkbox\" type=\"radio\""
	...
	// ... ou une case a cocher
		...
		return $label . '<label><input type="checkbox" '
			. ($valeur ? ' checked="checked"' : '') . " value=\"1\" name=\"$variable\" $disab/>"
	...
	return $label . ($lignes < 2
	// <html></html> empechera SPIP de modifier le contenu des <input> ou <textarea>
			? "<html><input name='$variable' value=\"" . htmlspecialchars($valeur) . "\" type='text' size='$len' $width $disab/></html>"
			: "<html><textarea rows='$lignes' name='$variable' $width$disab>" . htmlspecialchars($valeur) . '</textarea></html>'
		) . $comment . _VAR_OUTIL;
}
...
function description_outil_langues_callback($matches) {
	return _T($matches[1]);
}
...
function description_outil_liens_callback($matches) {
	...
	return '<a href="' . generer_url_ecrire('admin_couteau_suisse', 'cmd=descrip&outil=' . $matches[1]) . "\" id=\"href_$matches[1]\" onclick=\"javascript:return cs_href_click(this);\">$nom</a>";
}
...
function inc_description_outil_dist($outil_, $url_self, $modif = false) {
	...
	if (defined('_DIR_PLUGIN_PORTE_PLUME')
	 && (isset($outil['pipeline:porte_plume_barre_pre_charger']) || isset($outil['pipeline:porte_plume_cs_pre_charger']) || isset($outil['pipelinecode:porte_plume_barre_pre_charger']) || isset($outil['pipelinecode:porte_plume_cs_pre_charger']))
	...
	$res         = '';
	$nb_disabled = $nb_variables = 0;
	$variables   = array();
	for ($i = 0; $i < count($t); $i += 2)
		if (isset($t[$i + 1]) && strlen($var = trim($t[$i + 1]))) {
			// si la variable est presente on fabrique le input
			if (isset($cs_variables[$var])) {
				$res .= description_outil_une_variable($index + (++$nb_variables), $outil, $var, $t[$i], $modif);
				$variables[] = $var;
				if ($cs_variables[$var]['disabled'])
					++$nb_disabled;
			} else {
	...
	$res = pipeline('pre_description_outil', array(
		'outil' => $outil_,
		'texte' => $res,
		'actif' => $actif
	));
	...
	if ($actif) {
		...
		if ($nb_variables > 1)
			$res .= "<div class=\"cs_bouton\">$bouton</div>";
		else
			$res = str_replace(_VAR_OUTIL, $bouton, $res);
	}
	$res = "\n<div id='cs_inner_outil-$index' >" . str_replace(array(
		'<ul></ul>',
		_VAR_OUTIL
	), '', $res) . '</div>';
	...
	if (strpos($res, '<fusion') !== false)
		$res = str_replace(array(
			'</li></ul></div></fieldset><fusionradio>',
			'</div></fieldset><fusionradio>'
		), '', $res);
	...
	$res = str_replace(array(
		...
		couteauprive_T('votre_choix'),
		'<sup>(*)</sup>',
		cs_root_canonicalize(_DIR_PLUGIN_COUTEAU_SUISSE),
		defined('_SPIP19300') ? '' : couteauprive_T('detail_jquery3')
	), $res);
