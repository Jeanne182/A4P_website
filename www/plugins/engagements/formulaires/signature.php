...
function formulaires_signature_verifier_dist($id_article) {
	$erreurs = array();
	$oblis = array('session_nom','session_prenom', 'session_naissance','session_email', 'session_ecole', 'session_pays', 'session_entreprise');
	include_spip('base/abstract_sql');
	...
	foreach ($oblis as $obli) {
		if (!_request($obli)) {
			if($obli != 'session_ecole' || !_request('session_ecole_ajout')){
	...
	if ($prenom = _request('session_prenom') and strlen($prenom) < 2) {
		$erreurs['session_prenom'] = _T('form_indiquer_prenom');
	}

	if (!_request('ville')) {
		$erreurs['session_residence'] = _T('form_indiquer_residence');
	}

	if ($naissance = _request('session_naissance') and strlen($naissance)==0) {
		$erreurs['session_naissance'] = _T('info_obligatoire');
	} elseif (strlen($naissance) !=4 || !ctype_digit($naissance)) {
		$erreurs['session_naissance'] = _T('form_annee_promotion_format_incorrect');
	}

	if ($id_ecole = _request('session_ecole_ajout') and strlen($id_ecole) ==0) {
		if ($id_ecole = _request('session_ecole') and strlen($id_ecole) ==0) {
			$erreurs['session_ecole'] = _T('info_obligatoire');
		}
	}

	if (!_request('validation_ajout_ecole') && strlen(_request('session_ecole_ajout')) !=0){
		$erreurs['validation_ajout_ecole'] = _T('info_obligatoire');
	}

	if ($linkedin = _request('session_linkedin') and strlen($linkedin) !=0 and strpos($linkedin,'https://www.linkedin.com/in/')!==0) {
		$erreurs['session_linkedin'] = _T('form_indiquer_adresse_linkedin');		
	}
...
function formulaires_signature_traiter_dist($id_article) {
	...
	if (spip_connect()) {
		...
		if($id_ecole = _request('session_ecole_ajout') and strlen($id_ecole)==0){
			$id_ecole = _request('session_ecole');
		}
		$autorisation_infos_publiques='non';
		if(_request('validation_info_publiques')){
			$autorisation_infos_publiques='oui';
		}

		$membre_asso_alumni=NULL;
		if(_request('membre_asso_alumni')){
			$membre_asso_alumni=_request('membre_asso_alumni');
		}
		

		$reponse = $controler_signature($id_article,
			_request('session_prenom'),_request('session_nom'), _request('session_naissance'),_request('session_email'),$id_ecole, _request('session_pays'),$membre_asso_alumni,_request('session_linkedin'),
			_request('session_entreprise'),_request('session_fonction'), _request('pays'),_request('ville'),_request('code_postal'), $autorisation_infos_publiques,
...
function inc_controler_signature_dist($id_article, $prenom, $nom, $naissance, $mail, $id_ecole, $pays, $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $code_postal, $autorisation_infos_publiques, $message, $site, $url_site, $url_page) {
	...
	if ($id_ecole = _request('session_ecole_ajout') and strlen($id_ecole) > 0) {
		$id_ecole = sql_insertq("spip_ecoles_alumni", array('nom_ecole' => $id_ecole, 'id_pays' => $pays, 'statut' => 'prop'));
	}
	else{
		// $id_ecole_select = sql_select('id_ecole', 'spip_ecoles_alumni',array('id_ecole ='._request('session_ecole')));
		// $id_ecole = sql_fetch($id_ecole_select)['id_ecole'];
		$id_ecole = _request('session_ecole');
	}

	$row_ecoles = sql_fetsel('*', 'spip_ecoles_alumni', 'id_ecole=' . $id_ecole);
	$id_pays = $row_ecoles['id_pays'];
	$row_pays = sql_fetsel('*', 'spip_pays', 'id_pays=' . $id_pays);
	$row_pays['nom'] = explode("[fr]",$row_pays['nom'])[1];
	$row_pays['nom'] = explode("[en]",$row_pays['nom'])[0];

	$statut = "";
	if (!$ret = signature_a_confirmer($id_article, $url_page, $prenom, $nom, $naissance, $mail, $row_ecoles['nom_ecole'], $row_pays['nom'], $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $code_postal, $autorisation_infos_publiques, $site, $url_site, $message, 'fr'/*inutilise*/,
		$statut)
	) {
		return _T('petitions:form_pet_probleme_technique');
	}
	...
	if(_request('ville')){
		$id_gis = sql_insertq("spip_gis", array('titre' => _request('ville'), 'lat' => _request('lat'), 'lon' => _request('lon'), 'zoom' => _request('zoom'), 'pays' => _request('pays'), 'ville' => _request('ville'), 'code_postal' => _request('code_postal')));
	}
	...
	signature_modifier($id_signature,
		array(
			...
			'id_ecole' => $id_ecole,
			'membre_asso_alumni' => $membre_asso_alumni,
			'linkedin' => $linkedin,
			"organisation" => $entreprise,
			"fonction_organisation" => $fonction,
			"id_gis" => $id_gis,
			"autorisation_infos_publiques" => $autorisation_infos_publiques,
			// 'pays_alumni' => $pays,
			'message' => $message,
			// 'nom_site' => $site,
			// 'url_site' => $url_site,			
...
function signature_a_confirmer($id_article, $url_page, $prenom, $nom, $naissance, $mail, $id_ecole, $pays, $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $code_postal, $autorisation_infos_publiques, $site, $url, $msg, $lang, &$statut) {
	...
	$r = _T('petitions:form_pet_mail_confirmation',
		array(
			...
			"pays_organisation" => $pays_entreprise,
			"autorisation_infos_publiques" => $autorisation_infos_publiques,
			'nom_site' => $site,
			'url_site' => $url,
			'url' => $url_page,
