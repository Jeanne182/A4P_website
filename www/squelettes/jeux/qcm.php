...
function qcm_affiche_la_question(&$qcms, $indexJeux, $indexQCM, $gestionPoints) {
  ...
  if (!jeux_form_correction($indexJeux)){
	...
	// S'il y a trop de choix, utiliser une liste a la place des boutons radio
		...
		// boutons radio
		foreach($qcms[$indexQCM]['propositions'] as $i=>$valeur) 
			$codeHTML .= "<label class='container-checkbox selector text-black text-justify'  for='{$idInput}-$i'>&nbsp;$valeur<input type='radio' class='jeux_radio qcm_radio' aria-checked='true' name='$nameInput' value='$i' id='{$idInput}-$i' /><span class='checkmark selector-checkmark'></span></label>" . ($i % $nbcol?' &nbsp; ':'<br />');
	}
