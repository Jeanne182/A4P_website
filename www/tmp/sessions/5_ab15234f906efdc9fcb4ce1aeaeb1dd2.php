<?php
$GLOBALS['visiteur_session']['prefs'] = array (
  'couleur' => 9,
  'display' => 2,
  'display_navigation' => 'navigation_avec_icones',
  'display_outils' => 'oui',
  'cnx' => '',
);
$GLOBALS['visiteur_session']['id_auteur'] = 5;
$GLOBALS['visiteur_session']['nom'] = 'Bruno Altieri';
$GLOBALS['visiteur_session']['bio'] = '';
$GLOBALS['visiteur_session']['email'] = 'bruno.altieri@sciops.esa.int';
$GLOBALS['visiteur_session']['nom_site'] = '';
$GLOBALS['visiteur_session']['url_site'] = '';
$GLOBALS['visiteur_session']['login'] = 'baltieri';
$GLOBALS['visiteur_session']['statut'] = '0minirezo';
$GLOBALS['visiteur_session']['webmestre'] = 'non';
$GLOBALS['visiteur_session']['maj'] = '2020-10-05 21:13:37';
$GLOBALS['visiteur_session']['pgp'] = '';
$GLOBALS['visiteur_session']['en_ligne'] = '2020-10-05 21:13:37';
$GLOBALS['visiteur_session']['source'] = 'spip';
$GLOBALS['visiteur_session']['lang'] = '';
$GLOBALS['visiteur_session']['auth'] = 'spip';
$GLOBALS['visiteur_session']['hash_env'] = 'b3b7cd3e0d8f26516ea51f80cc3b509f';
$GLOBALS['visiteur_session']['ip_change'] = true;
$GLOBALS['visiteur_session']['date_session'] = 1601969632;
?>
